...
zeigt kräftigen Punkt, wenn ein Tier ein Geschenk bereit hat. Reine UI, keine Save-Änderung. Phase 81 (§9.83): visueller Skill-Tree (ui/SkillTree.svelte) — Prereq-Baum als radialer Reben-Knotengraph, Knoten-Zustände aus skillStatus, Klick→Detailkarte mit Lernen/MAX; SkillsPanel zeigt Baum+Legende statt Liste. Phase 85 (§9.87): Skill-Tree 5× erweitert (15→75 Skills, echte Verzweigungen + 3 neue Effekte growth/sellPrice/offline). Phase 86 (§9.88): Skill-Tree als eingerückte Liste statt Radial-Diagramm (Spieler-Feedback „zu klein/kaum klickbar am Browser") — SkillTree.svelte entfernt, SkillsPanel rendert gruppierte DFS-Liste mit Tiefen-Einrückung, voll klickbare Zeilen. Phase 87 (§9.89): veraltete „Füttern"-Texte auf das Geschenk-System korrigiert (Ziel-why + 2 Kommentare). Phase 88 (§9.90): Relikt-Bünde (data/relicSets.ts) — 4 themenbasierte Sammel-Sets über die Expeditions-Relikte, geben flachen dauerhaften Bonus zusätzlich zu Einzel-Relikt-Boni sobald man je 1× aller Mitglieder besitzt (rein aus state.relics abgeleitet, kein Save-Feld, übersteht Prestige+Weltensaat), relicSetBonus an denselben 5 Modifier-Stellen wie relicBonus, UI-Sektion im ExpeditionPanel + relicSetGoal-Nudge. Phase 89 (§9.91): neue Pflanzen-Kategorie „Göttlich" (goettlich) jenseits von Kosmisch — 6 Pflanzen (Ätherblüte→Allschöpfungsblüte), bewusst OHNE unlockParcel (nur Einnahmen-Gate, direkt kaufbar), eigene growth-Spezialisierung + distinkte Gold/Weiß-Sprites, voll datengetrieben über plantById. Keine Save-Änderung. Phase 90 (§9.92): Expeditionen vertieft — mythischer Relikt-Tier (3 Relikte) + 2 tiefe Weltensaat-Ziele (Weltenriss 12h/Schöpfungsquelle 48h) als einzige Quelle, neuer Mythischer Bund + grande Allumfassende-Sammlung-Capstone (Vollständige Sammlung auf die 8 Originale fixiert → kein Bonus-Entzug), save-safe (relics-Record, datengetrieben). Phase 91 (§9.93): zweiter paralleler Expeditions-Slot (activeExpedition2, ab 2 Weltensaat-Resets via expeditionSlots) — rückwärtskompatibel (Slot-Param Default 0, Slot 0 = altes activeExpedition), UI rendert Block je Slot + „Slots X/Y", Hud/goals/App-Toast prüfen beide Slots. SAVE_VERSION 39 (Migration v38→v39 defaultet activeExpedition2=null). Phase 92 (§9.94): Balance-Audit (Sims/Diagnose + Time-to-next-unlock-Kurve) — Spiel gesund, späte Kosmos/Cannabis-Stalls sind parzellen-gegatet (gewollt); Fund: Göttlich-Leiter (ohne Parzellen-Gate) hatte 10–50×-Unlock-Lücken gg. ~2,5× Profit → ~18× Stall/Pflanze, Fix auf ~3×-Stufen (1e48→3e47 … 2e56→1e50, T-Ratio ~1,2–1,4), reiner Buff + neue Pacing-Invariante im Test. Keine Save-Änderung. Phase 93 (§9.95): Kampagne bis ins neueste Endgame verlängert — 3 Kapitel (Mythischer Fund/Bund → Göttlicher Garten via maxUnlockEarned≥3e47) führen zu mythischen Relikten + göttlichen Pflanzen; save-safe ohne Version-Bump (linear angehängt, erstes Kapitel braucht neuen Inhalt → keine rückwirkende Belohnungsflut). Phase 94 (§9.96): Willkommen-zurück-Toast zählt jetzt zurückgekehrte Expeditionen + bereite Tier-Geschenke mit und meldet ehrlich, wenn der Offline-Cap die Abwesenheit begrenzt hat (nennt Nachteule/Sternenschlaf/Sternwarte/Sternenuhr). Reine UI, keine Save-Änderung. Phase 95 (§9.97): Offline-Nachholen entblockiert — processHelpers hatte einen quadratischen findIndex-Scan (O(Beete²)/Chunk) plus yieldMultiplier-Neuberechnung pro Ernte; Fix via Vorwärts-Cursor + level-invalidiertem Multiplikator-Cache (byte-identisches Verhalten gegen Referenz mit festem Seed). Endgame 8h: 2013→182 ms, 70h-Cap: ~17,6→~1,6 s weiße Seite beim Laden. Regressionstest prüft Skalierungsverhältnis (mutationsgetestet). Keine Save-Änderung. Phase 96 (§9.98): CI-Riegel (.github/workflows/ci.yml prüft jeden Push/PR; deploy.yml führt check+test VOR dem Build aus → kaputter Stand geht nie live) + Boot-Screen direkt in index.html (wird gemalt bevor das blockierende initGame() läuft, main.ts entfernt ihn nach dem Mount). Keine Save-Änderung. Stand SAVE_VERSION 39.
